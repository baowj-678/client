...
impl ParentStatusSyncer {
        ...
        for parent in add_parents.iter() {
            ...
            match status.get_mut(&ip) {
                ...
                Some(mut status_element) => {
                    ...
                    info!(
                        "[baowj] update register to: {}:{}, ref: {}",
                        &ip, &port, status_element.reference
                    );
                }
            }
                    ...
                    match client.sync_parent_status(request).await {
                        Ok(response) => {
                            ...
                            status.entry(parent.host.unwrap().ip).and_modify(|v| {
                                if !v.fixed {
                                    info!(
                                        "[baowj] sync_parent_status, update {} status: {}",
                                        host.ip,
                                        ByteSize(response)
                                    );
                                    (*v).status = response;
                        ...
                        Err(error) => {
                            info!(
                                "[baowj] sync_parent_status, failed to connect: {}, err: {:?}",
                                host.ip, error
                            );
                        }
                    };
    ...
    pub fn init_test_config(&self) {
        ...
        config.parent_selector.hosts.iter().for_each(|host| {
            ...
            info!(
                "[baowj] init_test_config insert ip: {}, port: {}",
                host.ip.to_string(),
                host.port
            );
        });
    ...
    pub fn get_parents_status(&self, parents: &Vec<CollectedParent>) -> Vec<u64> {
        ...
        parents.iter().for_each(|parent| {
            result.push(match status.get(&parent.host.clone().unwrap().ip) {
                None => ByteSize::from_str("1GiB").unwrap().as_u64(),
                Some(value) => value.clone().status,
            })
        });
